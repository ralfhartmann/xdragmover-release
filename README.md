...
Settings' General tab has a "Check for Updates" checkbox, off by default since this is the app's only networking code. When enabled, XDragMover checks the public release repo for a newer version 5 minutes after launch and once a day thereafter; if one is found, a dialog offers a direct download link to the new `.dmg`. Independent of that toggle, a check can also be triggered on demand at any time (US-22) via the menu bar item's "Check for Updates…" or Settings' "Check Now" button — unlike the silent scheduled check, a manual one always reports its outcome: a newer version, already up to date, or a failed check (e.g. no network).
...
| Check Now (button, next to the toggle above) | Runs an update check immediately, regardless of the toggle above; always reports its outcome (US-22). Also available from the menu bar item ("Check for Updates…") | ✅ Implemented |
