...
Settings' Gestures tab has a "Snap Window to Screen Edge on Release" checkbox, off by default (US-20). When enabled, while moving a window (feature 1 above), holding the mouse within 5px of a screen edge for 1 second and then releasing snaps the window to that edge's half (if released in the middle third of the edge, lengthwise) or the nearest corner's quarter (if released in one of the two outer thirds). Moving the mouse again — even slightly — resets the 1-second requirement; releasing anywhere else just drops the window normally, unchanged from before.
...
| Snap Window to Screen Edge on Release | Releasing a dragged window near a screen edge, after a 1-second dwell within 5px of it, snaps it to a half or quarter of the screen (US-20). Default: off | ✅ Implemented |
